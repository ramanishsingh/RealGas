...
def test_binary_virial():
    ...
    assert np.isclose(mixture.H_R_RT(mixture.i.cas_number, 1., P, T), I.H_R_RT_expr(P, T)), 'HR for binary virial does not converge to unary'
    assert np.isclose(mixture.G_R_RT(mixture.i.cas_number, 1., P, T), I.G_R_RT_expr(P, T)), 'GR for binary virial does not converge to unary'
    assert np.isclose(mixture.S_R_R(mixture.i.cas_number, 1., P, T), I.S_R_R_expr(P, T)), 'Sr for binary virial for component i does not converge to unary'
    assert np.isclose(mixture.H_R_RT(mixture.j.cas_number, 1., P, T), J.H_R_RT_expr(P, T)), 'Hr for binary virial of component j does not converge to virial'
    assert np.isclose(mixture.G_R_RT(mixture.j.cas_number, 1., P, T), J.G_R_RT_expr(P, T)), 'GR for binary virial of component j does not converge to unary'
    assert np.isclose(mixture.S_R_R(mixture.j.cas_number, 1., P, T), J.S_R_R_expr(P, T)), 'SR for binary virial of component j does not converge to unary'
